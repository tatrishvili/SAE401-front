import badgeFlowers from '@/assets/images/badge_flowers.png'
import badgeGlasses from '@/assets/images/badge_glasses.png'
import badgeCacheOreille from '@/assets/images/badge_cacheoreille.png'
import badgeMustache from '@/assets/images/badge_mustache.png'
import badgeMonocle from '@/assets/images/badge_monocle.png'
import badgePirate from '@/assets/images/badge_pirate.png'
import badgeSunglasses from '@/assets/images/badge_sunglasses.png'
import badgeDeer from '@/assets/images/badge_deer.png'
import badgeChristmas from '@/assets/images/badge_christmas.png'
...
export const normalizeBadge = (badge, fallbackIndex = 0) => ({
  ...
  image:
    ...
    badge.asset ??
    BADGE_IMAGE_BY_ID[
      String(badge.id ?? badge.slug ?? badge.code ?? badge.name ?? '').toLowerCase()
    ] ??
...
export const normalizeBadgesArray = (value) =>
  Array.isArray(value)
    ? value.map((badge, index) => normalizeBadge(badge, index)).filter((badge) => badge.id)
    : []
...
export const resolveBadgeImage = (badge, index = 0) =>
  ...
  badge?.badgeImage ??
  badge?.asset ??
  BADGE_IMAGE_BY_ID[String(badge?.id ?? badge?.slug ?? badge?.code ?? '').toLowerCase()] ??
